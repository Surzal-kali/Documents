...
### 2.)  Marlinspike=55 (Apache, Ubuntu 16.04)
        ...
        #### The Guest Bin:
        It has mysql. The site dynamically loads all of its content off an api call. This could be my injection point for the actual root account. In addition, the vm sits on Proxmox, but also has a container with ubuntu desktop. With an outdated version of ubuntu 16.04. So there are likely vulnerabilities Theres something in the underlying system that can be exploited.
        ...
        So we can read the mysql configuration files, and likely get the credentials for the mysql database. From there, we can likely get a reverse shell through the wordpress installation. The virtual "virtual" network is known as Windows Network, so both the container and the vm are concurrently running. This inception bullshit makes my head hurt.

        After checking the system files why yes captain I can read the entire site likes its a maze on a cracker jack box. ITS G-R-E-A-T. We can simply add a user to the  wordpress installation and log in like normal, because the guest account can read the mysql configuration files, and get the credentials for the wordpress, or even reset them. However up on even FURTHER testing, i've concluded that the mysql database is running elsewhere on the system, not associated with the wordpress installation so we have no access to it yet. Nor can we execute scripts from the machine that are custom, however we CAN connect to a web server that we control, so we can serve a web payload and execute it on the machine, which is really cool. So we can use the guest account to connect to a web server that we control, and serve a web payload that will give us a reverse shell on the machine. This is really cool, as it allows us to bypass the shitty text editor installed on the machine, and instead make something amazing :D.
